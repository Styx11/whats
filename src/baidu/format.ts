...
// from en to zh
export const defaultFormat = (rawData: any) => {
...
export const chineseFormat = (rawData: any) => {
	const data = JSON.parse(rawData);
	const errCode = data.error_code;
	if (errCode && errCode !== '52000') {
		throw new Error('或许是网络问题，请重试');
	...
	const symbols = rawSymbols.map((rs: any) => {
		const word_symbol = rs.word_symbol || '';
		const rawParts = rs.parts || [];
		const parts = rawParts.map((rp: any) => {
			const means = rp.means.map((m: any) => m.text || '');// simplified output
			return {
